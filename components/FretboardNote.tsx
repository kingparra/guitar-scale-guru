...
const FretboardNote: React.FC<FretboardNoteProps> = React.memo(({
    note, x, y, fontScale, isRoot, isTonicChordTone,
    isCharacteristic, isInRun, runNotesLookup, finger, sequenceNumber
}) => {
    ...
    const fillColor = fret === 0 ? COLORS.openString : (isRoot ? COLORS.root : COLORS.tone);
    const textColor = fret === 0 ? '#000' : COLORS.bgPrimary;

    return (
        <g opacity={opacity} style={{ transition: 'opacity 0.3s ease-in-out' }}>
            {/* Note Marker (Circle or Triangle for Root) */}
            {isRoot ? (
                <path d={`M ${x} ${y - r} L ${x + r * 0.866} ${y + r * 0.5} L ${x - r * 0.866} ${y + r * 0.5} Z`}
                    fill={fillColor} stroke={isCharacteristic ? COLORS.characteristicOutline : 'none'} strokeWidth={3}
                />
            ) : (
                <circle cx={x} cy={y} r={r} fill={fillColor} stroke={isCharacteristic ? COLORS.characteristicOutline : 'none'} strokeWidth={3} />
            )}
            ...
            {/* Finger Number (top left) - always just text now */}
            {finger && (
                <text
                    x={x - (20 * fontScale)}
                    y={y - (20 * fontScale)}
