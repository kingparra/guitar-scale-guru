...
const NotationAnalyzer: React.FC<NotationAnalyzerProps> = ({
    ...
    return (
        <>
            <Card>
                <div className="p-4">
                    ...
                    <p className="text-lg max-w-3xl mx-auto mb-6 text-center" style={{ color: COLORS.textSecondary }}>
                        Upload a multi-page PDF or an image of your score, and the AI will analyze the entire document to suggest the best scale.
                    </p>
            ...
            {isAnalyzing && <Card><div className="flex flex-col items-center justify-center p-16"><div className="animate-spin rounded-full h-12 w-12 border-b-4 border-cyan-400"></div><p className="mt-4 text-lg font-semibold" style={{ color: COLORS.textPrimary }}>Analyzing notation... this may take a moment.</p></div></Card>}
                        ...
                        <div className="space-y-6">
                            {results.map((result, index) => (
                                <div key={index} className="bg-black/20 p-6 rounded-lg border border-purple-400/20">
                                    ...
                                    <button onClick={() => onGenerateFromAnalysis(result)} className="mt-2 bg-gradient-to-r from-purple-500 to-indigo-600 text-white font-bold py-2 px-6 rounded-lg shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-300">
                                        Generate Materials
                                    </button>
                                </div>
                            ))}
