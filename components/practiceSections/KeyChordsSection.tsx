            ...
            <div className="space-y-6">
                ...
                {keyChords.progressions.map((p) => (
                    <div key={p.name} className="mt-4">
                        ...
                        <div className="flex overflow-x-auto space-x-4 items-start p-2 rounded-md scrollbar-thin scrollbar-thumb-purple-400/50 scrollbar-track-transparent">
                            {p.chords.map((chord, index) => (
                                <React.Fragment key={`${chord.name}-${index}`}>
                                    <div className="flex flex-col items-center gap-2 flex-shrink-0 w-48">
                                        <DegreePill degree={chord.degree} />
                                        <ChordFretboardDiagram
                                            chord={chord}
                                            fontSize={fontSize}
                                        />
                                        <p className="font-semibold text-center h-10">
                                            {chord.name}
                                        </p>
                                    </div>
