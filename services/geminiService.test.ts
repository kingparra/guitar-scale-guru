...
import { generateOverview, analyzeMusicNotationImage } from './geminiService';
import { GoogleGenAI } from '@google/genai';
import { notationAnalysisPrompt, getOverviewPrompt } from './prompts';
...
jest.mock('./prompts', () => ({
    ...
    getOverviewPrompt: jest.fn().mockReturnValue('mocked-overview-prompt'),
    getResourcesPrompt: jest.fn().mockReturnValue('mocked-resources-prompt'),
    getPracticePrompt: jest.fn().mockReturnValue('mocked-practice-prompt'),
}));

// Create typed mocks
const mockGenerateContent = jest.fn();
const mockGetOverviewPrompt = getOverviewPrompt as any;
...
beforeEach(() => {
    ...
    mockGetOverviewPrompt.mockClear();
});
...
describe('geminiService', () => {
    describe('generateOverview', () => {
        it('should call the prompt generator and the Gemini API with the correct arguments', async () => {
            ...
            await generateOverview('E', 'Harmonic Minor');

            expect(mockGetOverviewPrompt).toHaveBeenCalledWith(
                'E',
                'Harmonic Minor'
            );
            ...
            expect(callArgs.contents).toBe('mocked-overview-prompt');
        ...
        it('should return parsed JSON data on success', async () => {
            ...
            const result = await generateOverview('C#', 'Lydian');
            expect(result).toEqual(mockResponse);
        });

        it('should throw an error if the API call fails', async () => {
            mockGenerateContent.mockRejectedValue(new Error('API Error'));
            await expect(generateOverview('A', 'Major')).rejects.toThrow(
    ...
    describe('analyzeMusicNotationImage', () => {
        it('should call the multimodal API with the correct prompt and image data', async () => {
            ...
            expect(callArgs.contents.parts[0].text).toBe(
                'mocked-notation-prompt'
            );
            expect(callArgs.contents.parts[1].inlineData.data).toBe(imageData);
            expect(callArgs.contents.parts[1].inlineData.mimeType).toBe(
                mimeType
            );
            expect(callArgs.model).toBe('gemini-2.5-pro');
        });
        ...
        it('should return parsed JSON array on success', async () => {
            ...
            const result = await analyzeMusicNotationImage(
                'dummyData',
                'image/jpeg'
            );
            expect(result).toEqual(mockResponse);
